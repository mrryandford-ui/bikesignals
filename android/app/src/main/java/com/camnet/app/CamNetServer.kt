...
class CamNetServer(port: Int, private val assets: AssetManager) : NanoWSD(port) {
    ...
    // ── WebSocket connection with per-session state ───────────────
            ...
            when (msg.optString("type")) {
                ...
                "rejoin-room" -> {
                    ...
                    room.cameras.forEach { (camId, cam) ->
                        trySend(this, jObj("type" to "camera-joined", "cameraId" to camId, "cameraName" to (cam.cameraName ?: camId)))
                    }
                ...
                "join-room" -> {
                    ...
                    val room = rooms[rId]
                    if (room == null) {
                        trySend(this, jObj("type" to "error", "code" to "NO_ROOM",
                            "message" to "Room not found. Check the code and try again."))
                        return
                    }
                    val name = msg.optString("cameraName").trim().ifEmpty { "Camera ${room.cameras.size + 1}" }
                    // Remove stale entry with same name
                    room.cameras.entries.firstOrNull { it.value.cameraName == name }?.let {
                        room.cameras.remove(it.key)
                        room.viewer?.let { v -> trySend(v, jObj("type" to "camera-left", "cameraId" to it.key)) }
                    }
                    ...
                    room.viewer?.let { trySend(it, jObj("type" to "camera-joined", "cameraId" to id, "cameraName" to name)) }
                }
    ...
    private fun serveApiInfo(): Response {
        ...
        val allArr = ips.joinToString(",") { """{"address":"$it","family":"IPv4","internal":false}""" }
        val json = """{"lanIP":${if (first != null) "\"$first\"" else "null"},"allIPs":[$allArr]}"""
    ...
    private fun lanIP(): String? = localIPs().firstOrNull()
    ...
    private fun mimeOf(path: String) = when {
        ...
        path.endsWith(".webmanifest") || path.endsWith(".json") -> "application/json"
